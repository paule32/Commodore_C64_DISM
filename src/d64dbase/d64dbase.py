...
# ---------------------------------------------------------------------------
# dBase Stage 34 / Form OOP + WFM
# ---------------------------------------------------------------------------
# Additive Form front-end used by the visual designer and by the compiler.
# It intentionally keeps the existing dBase parser untouched for non-WFM
# sources. CLASS ... OF FORM is lowered to the Qt5 bridge calls below.
...
def parse_dbase_wfm(source: str, *, filename: str = "<WFM>") -> DBaseWfmForm:
    ...
    WITH/ENDWITH, direct member assignments and NEW FONT(...).
    """
...
def _wfm_asm_bytes(text: str) -> str:
    data = str(text).encode("utf-8") + b"\0"
    return ", ".join(str(b) for b in data)


def _wfm_int(props: Dict[str, object], name: str, default: int) -> int:
...
def _wfm_prop(props: Dict[str, object], name: str, default=None):
    for key, value in props.items():
        if key.casefold() == name.casefold(): return value
    return default


_stage33_compile_dbase_to_assembly = compile_dbase_to_assembly


def _compile_wfm_to_assembly(source: str, *, filename: str, target: str, windows_application_mode: str) -> DBaseCompileResult:
    model = parse_dbase_wfm(source, filename=filename)
    base = _stage33_compile_dbase_to_assembly(
        "", filename=filename, target=target, windows_application_mode=windows_application_mode
    )
    is64 = str(target).casefold() in {"pe64", "win64", "pe32+", "x64"}
    ptr = "qword" if is64 else "dword"
    imports = (
        "DBaseQtFormCreate", "DBaseQtControlCreate", "DBaseQtWidgetSetGeometry",
        "DBaseQtWidgetSetText", "DBaseQtWidgetSetBackColor", "DBaseQtWidgetSetBorderColor",
        "DBaseQtWidgetSetBorderWidth", "DBaseQtWidgetSetRadius", "DBaseQtWidgetSetFont",
        "DBaseQtFormOpen",
    )
    import_text = "".join(f'import {name}, "d64qt5.dll", "{name}"\n' for name in imports)
    marker = 'import DBaseQtShutdown, "d64qt5.dll", "DBaseQtShutdown"\n'
    assembly = base.assembly.replace(marker, marker + import_text, 1)

    labels: List[Tuple[str, str]] = []
    label_count = 0
    def text_label(value: object) -> Tuple[str, int]:
        nonlocal label_count
        txt = str(value)
        label = f"__dbase_wfm_text_{label_count}"; label_count += 1
        labels.append((label, txt))
        return label, len(txt.encode("utf-8"))
    def slot_for(path: str) -> str:
        safe = re.sub(r"[^A-Za-z0-9_]", "_", _wfm_normalize_path(path))
    ...
    if is64:
        lines += [f"    mov rcx, {class_label}", f"    mov edx, {class_len}", "    sub rsp, 40", "    call DBaseQtFormCreate", "    add rsp, 40", f"    mov qword ptr [{form_slot}], rax"]
    else:
        lines += [f"    push {class_len}", f"    push {class_label}", "    call DBaseQtFormCreate", "    add esp, 8", f"    mov dword ptr [{form_slot}], eax"]

    fw = _wfm_int(model.properties, "Width", 400); fh = _wfm_int(model.properties, "Height", 400)
    fl = _wfm_int(model.properties, "Left", 200); ft = _wfm_int(model.properties, "Top", 200)
    def emit_geometry(slot: str, l:int,t:int,w:int,h:int):
        if is64:
            lines.extend([f"    mov rcx, {ptr} ptr [{slot}]", f"    mov edx, {l}", f"    mov r8d, {t}", f"    mov r9d, {w}", "    sub rsp, 48", f"    mov dword ptr [rsp+32], {h}", "    call DBaseQtWidgetSetGeometry", "    add rsp, 48"])
        else:
            lines.extend([f"    push {h}", f"    push {w}", f"    push {t}", f"    push {l}", f"    push dword ptr [{slot}]", "    call DBaseQtWidgetSetGeometry", "    add esp, 20"])
    def emit_string(fn: str, slot: str, value: object):
        label, n = text_label(value)
        if is64:
            lines.extend([f"    mov rcx, {ptr} ptr [{slot}]", f"    mov rdx, {label}", f"    mov r8d, {n}", "    sub rsp, 40", f"    call {fn}", "    add rsp, 40"])
        else:
            lines.extend([f"    push {n}", f"    push {label}", f"    push dword ptr [{slot}]", f"    call {fn}", "    add esp, 12"])
    def emit_int(fn: str, slot: str, value: int):
        if is64:
            lines.extend([f"    mov rcx, {ptr} ptr [{slot}]", f"    mov edx, {int(value)}", "    sub rsp, 40", f"    call {fn}", "    add rsp, 40"])
        else:
            lines.extend([f"    push {int(value)}", f"    push dword ptr [{slot}]", f"    call {fn}", "    add esp, 8"])

    emit_geometry(form_slot, fl, ft, fw, fh)
    # Controls are already in source order, so every CONTAINER exists before its children in designer output.
    for control in sorted(model.controls, key=lambda c: c.order):
        slot = slot_for(control.path)
        parent_slot = form_slot if control.parent_path.casefold() == "this" else slot_for(control.parent_path)
        typelabel, typelen = text_label(control.class_name)
        if is64:
            lines.extend([f"    mov rcx, {typelabel}", f"    mov edx, {typelen}", f"    mov r8, {ptr} ptr [{parent_slot}]", "    sub rsp, 40", "    call DBaseQtControlCreate", "    add rsp, 40", f"    mov qword ptr [{slot}], rax"])
        else:
            lines.extend([f"    push dword ptr [{parent_slot}]", f"    push {typelen}", f"    push {typelabel}", "    call DBaseQtControlCreate", "    add esp, 12", f"    mov dword ptr [{slot}], eax"])
        props = control.properties
        emit_geometry(slot, _wfm_int(props,"Left",0), _wfm_int(props,"Top",0), _wfm_int(props,"Width",120), _wfm_int(props,"Height",30))
        text = _wfm_prop(props, "Text", None)
        if text is not None: emit_string("DBaseQtWidgetSetText", slot, text)
        back = _wfm_prop(props, "BackColor", None)
        if back is not None: emit_string("DBaseQtWidgetSetBackColor", slot, back)
        bcolor = _wfm_prop(props, "BorderColor", None)
        if bcolor is not None: emit_string("DBaseQtWidgetSetBorderColor", slot, bcolor)
        bwidth = _wfm_prop(props, "BorderWidth", None)
        if bwidth is not None: emit_int("DBaseQtWidgetSetBorderWidth", slot, int(float(bwidth)))
        radius = _wfm_prop(props, "Radius", None)
        if radius is not None: emit_int("DBaseQtWidgetSetRadius", slot, int(float(radius)))
        if control.font is not None:
            font = control.font; flabel, flen = text_label(font.family)
            if is64:
                # handle,family,len,size,bold,italic,underline,strikeout
                lines.extend([f"    mov rcx, {ptr} ptr [{slot}]", f"    mov rdx, {flabel}", f"    mov r8d, {flen}", f"    mov r9d, {font.size}", "    sub rsp, 72", f"    mov dword ptr [rsp+32], {1 if font.bold else 0}", f"    mov dword ptr [rsp+40], {1 if font.italic else 0}", f"    mov dword ptr [rsp+48], {1 if font.underline else 0}", f"    mov dword ptr [rsp+56], {1 if font.strikeout else 0}", "    call DBaseQtWidgetSetFont", "    add rsp, 72"])
            else:
                lines.extend([f"    push {1 if font.strikeout else 0}", f"    push {1 if font.underline else 0}", f"    push {1 if font.italic else 0}", f"    push {1 if font.bold else 0}", f"    push {font.size}", f"    push {flen}", f"    push {flabel}", f"    push dword ptr [{slot}]", "    call DBaseQtWidgetSetFont", "    add esp, 32"])

    if is64:
        lines += [f"    mov rcx, qword ptr [{form_slot}]", "    sub rsp, 40", "    call DBaseQtFormOpen", "    add rsp, 40"]
    else:
        lines += [f"    push dword ptr [{form_slot}]", "    call DBaseQtFormOpen", "    add esp, 4"]
    body = "\n".join(lines) + "\n"
    # The console shell must not be shown for a WFM form. Build controls at exactly this point.
    old_show = "    call DBaseQtEnsureDefaultMenu\n    call DBaseQtShowWindow\n"
    if old_show in assembly:
        assembly = assembly.replace(old_show, body, 1)
    else:
        assembly = assembly.replace("    call DBaseQtShowWindow\n", body, 1)
    data = ["", "; Stage 34 WFM object handles", f"{form_slot}:", "    dq 0" if is64 else "    dd 0"]
    for c in model.controls:
        data += [f"{slot_for(c.path)}:", "    dq 0" if is64 else "    dd 0"]
    for label, txt in labels:
        data += [f"{label}:", f"    db {_wfm_asm_bytes(txt)}"]
    assembly = assembly.rstrip() + "\n" + "\n".join(data) + "\n"
    notes = tuple(base.notes) + (
        "dBase-Ausbaustufe 34: CLASS <Name> OF FORM, PROPERTY, THIS, NEW, WITH/ENDWITH und *.wfm.",
        "PUSHBUTTON und CONTAINER werden als native Qt-Widgets erzeugt; verschachtelte THIS-Pfade bilden Parent/Child-OOP ab.",
        "WFM-Dateien sind zugleich lesbare OOP-Quellen und die Persistenzform des visuellen Formulardesigners.",
    )
    return DBaseCompileResult(
        assembly=assembly, target=base.target, windows_application_mode=windows_application_mode,
        source_kind="form", notes=notes, warnings=base.warnings, linked_assembly_files=base.linked_assembly_files,
        linked_pe32_modules=base.linked_pe32_modules, linked_object_files=base.linked_object_files,
        frontend=base.frontend, statements=tuple(model.controls), transcript="", debug_transcript="",
        uses_debug_output=False, variables=base.variables, external_functions=base.external_functions,
    )
...
def compile_dbase_to_assembly(
    ...
    *,
    ...
    if re.search(r"(?im)^\s*CLASS\s+[A-Za-z_]\w*\s+OF\s+FORM\s*$", str(source)):
        return _compile_wfm_to_assembly(
            source, filename=filename, target=target, windows_application_mode=windows_application_mode
        )
    return _stage33_compile_dbase_to_assembly(
        source, filename=filename, target=target, windows_application_mode=windows_application_mode
    )

for _name in ("DBaseWfmFont", "DBaseWfmControl", "DBaseWfmForm", "parse_dbase_wfm"):
    if _name not in __all__:
        __all__.append(_name)
