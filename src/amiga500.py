...
class AmigaBootProgram:
    """Bootfähiges ADF mit direkt ausführbarem 68000-Bootblock."""
...
def _bootable_adf(code: bytes) -> Tuple[bytes, bytes]:
    maximum = BOOT_BLOCK_SIZE - BOOT_CODE_OFFSET
    if len(code) > maximum:
        raise AmigaAssemblerError(
            "Der Standalone-Bootcode ist zu groß: "
            f"{len(code)} Bytes; erlaubt sind höchstens {maximum} Bytes."
        )

    boot_block = bytearray(BOOT_BLOCK_SIZE)
    boot_block[0:4] = b"DOS\0"
    struct.pack_into(">I", boot_block, 8, AMIGA_DD_ROOT_BLOCK)
    boot_block[BOOT_CODE_OFFSET:BOOT_CODE_OFFSET + len(code)] = code
...
def assemble_amiga_boot_source(
    ...
    *,
    ...
    return AmigaBootProgram(
        ...
        code=program.code,
        entry_offset=BOOT_CODE_OFFSET + program.entry_offset,
        end_offset=BOOT_CODE_OFFSET + program.end_offset,
