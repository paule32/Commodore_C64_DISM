...
def compile_c_module_to_assembly(
    ...
    *,
    ...
    if normalized_target in {"c64", "c-64", "6510"}:
        ...
        target_macros.setdefault("__D64_GRAPHICS_DIRECT3D__" if str(graphics_backend).casefold().startswith("direct3") else "__D64_GRAPHICS_DIRECT2D__", 1)
    else:
        raise C64CError(
    ...
    *,
    ...
    try:
        if normalized_target in {"c64", "c-64", "6510"}:
            ...
            generated = _PE32CCodeGenerator(frontend, graphics_backend=graphics_backend, console_mode=not uses_graphics).generate()
        else:
            raise C64CError(f"Unbekanntes Compilerziel: {target}.", filename=filename)
